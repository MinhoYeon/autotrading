...
def get_ror(ticker, year, target_k, mo_ve_n):
    try:
        df = pybithumb.get_ohlcv(str(ticker))
        df = df[str(year)]
        # 목표가 설정
        df['range'] = (df['high'] - df['low']) * float(target_k)
...
# hprs = []
# for year in np.arange(2017, 2019, 1):
#     raw_data = {'ticker' : [],
#                 'hpr': []}
...
for ticker in tickers:
    for k in np.arange(0.1, 1.0, 0.1):
        hpr = get_ror(ticker, 2018, k, 5)
        #hprs.append((ticker, k, hpr))
#print(hprs)
        print("%s %.1f %f" % (ticker, k, hpr))
